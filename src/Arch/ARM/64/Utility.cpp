...
const void* ARM64PCCompute(ICapstone* capstone, const void* at, uint64_t disp)
{
    return (const char*)at + disp;
}

const void* ARM64LDRPCDispResolve(ICapstone* capstone, const void* at, bool bDerref)
{
    CsInsn insn = capstone->DisassembleOne(at);

    if (insn->id != ARM64_INS_LDR && insn->id != ARM64_INS_LDRSW && insn->id != ARM64_INS_PRFM)
        throw std::runtime_error(fmt::format("ARM64LDRPCDispResolve '{} {}': unexpected instruction", insn->mnemonic, insn->op_str));

    auto& ops = insn->detail->arm64.operands;

    if (insn->detail->arm64.op_count < 2)
        throw std::runtime_error(fmt::format("ARM64LDRPCDispResolve '{} {}': too few operands", insn->mnemonic, insn->op_str));
    ...
    if (!bHandled && ops[1].type == ARM64_OP_MEM) {
        offset = ops[1].mem.disp;
        bHandled = true;
    }

    if (!bHandled)
        throw std::runtime_error(fmt::format("ARM64LDRPCDispResolve '{} {}': unexpected operand types", insn->mnemonic, insn->op_str));

    const void* dstAddr = ARM64PCCompute(capstone, at, offset);

    if (!bDerref)
        return dstAddr;

    if (insn->id == ARM64_INS_LDRSW)
        return (const void*)(intptr_t)(*(int32_t*)dstAddr);

    return (const void*)(*(uintptr_t*)dstAddr);
}

const void* ARM64FarPcRelLEATryResolve(ICapstone* capstone, const void* at, bool bDerref)
{
    CsInsn adrpInsn = capstone->DisassembleOne(at, (uint64_t)at);
    if (adrpInsn->id != ARM64_INS_ADRP || adrpInsn->detail->arm64.op_count < 2)
        return nullptr;

    aarch64_reg Rd = adrpInsn->detail->arm64.operands[0].reg;
    uint64_t PageBase = adrpInsn->detail->arm64.operands[1].imm;
    ...
    capstone->InsnForEach(nextInsnStart, [&](const CsInsn& insn) {
        ...
        if (insn->id == ARM64_INS_ADD) {
            if (insn->detail->arm64.op_count >= 3 &&
                insn->detail->arm64.operands[1].reg == Rd &&
                insn->detail->arm64.operands[2].type == ARM64_OP_IMM) {

                res = (const void*)(PageBase + insn->detail->arm64.operands[2].imm);
                return !(bFound = true);
            }
        }
        ...
        switch (insn->id)
        {
        case ARM64_INS_LDR: case ARM64_INS_STR:
        ...
        if (bIsLoadStore) {
            if (insn->detail->arm64.op_count >= 2 &&
                insn->detail->arm64.operands[1].type == ARM64_OP_MEM &&
                insn->detail->arm64.operands[1].mem.base == Rd) {

                res = (const void*)(PageBase + insn->detail->arm64.operands[1].mem.disp);
                return !(bFound = true);
            }
        }
        ...
        // Explicit writes
        if (!bOverwritten && insn->detail->arm64.op_count > 0 &&
            insn->detail->arm64.operands[0].type == ARM64_OP_REG &&
            insn->detail->arm64.operands[0].reg == Rd &&
            (insn->detail->arm64.operands[0].access & CS_AC_WRITE)) {
            bOverwritten = true;
        }
